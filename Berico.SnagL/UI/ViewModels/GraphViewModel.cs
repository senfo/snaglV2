...
namespace Berico.SnagL.UI
{
    ...
    public class GraphViewModel : ViewModelBase
    {
        ...
        private NodeViewModelBase dragTargetNode = null;
                ...
                public ICommand MouseLeftButtonDown
                {
                    get
                    {
                        return new RelayCommand<MouseButtonEventArgs>(e =>
                        {
                            ...
                            {
                                // Check if we are in PanMode or not
                                if (IsPanMode)
                                    this.isPanning = true;
                                else
                                {
                                    ...
                                    //selectionStartPosition = e.GetPosition(graphViewPort);
                                    lastMousePosition = e.GetPosition(graphViewPort);
                                }

                                // If we are not in panmode, then we cen select nodes
                                // or zoom (if we do zoom in this manner)
                            }
                        });
                    }
                }
                        ...
                        return new RelayCommand<DetailedEventInformation>(e =>
                            {
                                ...
                                DetailedEventInformation eventInfo = e as DetailedEventInformation;
                                ...
                                if (!isPanning && isLeftButtonDown && dragTargetNode != null && !isNodeDragging)
                                    isNodeDragging = true;
                                else if (!isPanning && isLeftButtonDown && dragTargetNode == null && !isSelectingArea)
                                {
                                    isSelectingArea = true;
                                    selectionStartPosition = currentMousePosition;
                                }

                                // && GraphManager.Instance.DefaultGraphComponentsInstance.NodeSelector.SelectedNode != null
                                if (!isPanning && isNodeDragging)
                                {
                                    ...
                                    else // no nodes are currently selected
                ...
                public ICommand MouseLeftButtonUp
                {
                    get
                    {
                        return new RelayCommand<MouseButtonEventArgs>(e =>
                        {
                            ...
                            else if (this.isSelectingArea && selectionStartPosition != lastMousePosition)
                            {
                ...
                public ICommand MouseWheel
                {
                    get
                    {
                        return new RelayCommand<DetailedEventInformation>(e =>
                        {
                            ...
                            double zoomChange = 0.05 * System.Math.Sign(eventArgs.Delta);
